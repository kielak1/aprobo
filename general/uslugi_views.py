...
class UslugiTable(tables.Table):
    ...
    class Meta:
        model = uslugi
        fields = ("link", "id", "numer", "nazwa", "suma_budzetu")
...
class UslugiFilter(BaseDateRangeFilterSet):
    id = django_filters.CharFilter(lookup_expr="icontains", label="Id")
    nazwa = django_filters.CharFilter(lookup_expr="icontains", label="Nazwa")
    numer = django_filters.CharFilter(lookup_expr="icontains", label="Numer")
...
class WszystkieUslugi(LoginRequiredMixin, SingleTableMixin, FilterView):
    table_class = UslugiTable
    model = uslugi
    template_name = "uslugi-list.html"
    filterset_class = UslugiFilter
    ...
    def get_queryset(self):
        ...
        # Filtrowanie i agregacja budżetów na podstawie relacji ManyToMany z modelem Needs
